...
/// Cosmos Hub chain service using Cosmos REST API.
///
/// Uses BIP44 derivation (m/44'/118'/0'/0/0) for Cosmos addresses (cosmos1...).
/// Communicates via Cosmos SDK LCD (REST) API for balance and transaction queries.
/// Cosmos is the IBC hub chain powering inter-blockchain communication.
class CosmosService extends ChainService {
  final http.Client _client;
  final String _apiBaseUrl;

  CosmosService({http.Client? client, String? apiBaseUrl})
      : _client = client ?? http.Client(),
        _apiBaseUrl = apiBaseUrl ?? AppConstants.cosmosRestUrl;

  @override
  String get chainName => 'Cosmos';

  @override
  String get chainSymbol => 'ATOM';

  @override
  String get chainIcon => '\u{269B}'; // atom symbol

  @override
  int get decimals => 6;

  @override
  String get explorerUrl => 'https://www.mintscan.io/cosmos';
  ...
  Future<double> getBalance(String address) async {
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception('Cosmos API HTTP ${response.statusCode}: ${response.body}');
      }
      ...
      for (final balance in balances) {
        ...
        if (balanceData['denom'] == 'uatom') {
          final uatom = int.tryParse(balanceData['amount'] as String? ?? '0') ?? 0;
          // Convert uatom to ATOM (1 ATOM = 1,000,000 uatom)
          return uatom / 1000000.0;
        }
      }

      return 0.0;
    } catch (e) {
      throw Exception('Cosmos balance fetch failed: $e');
    }
  }

  @override
  Future<String> sendTransaction(String to, double amount) async {
    // Full Cosmos transaction requires:
    // 1. Query account number and sequence via /cosmos/auth/v1beta1/accounts
    // 2. Construct MsgSend transaction
    // 3. Sign with secp256k1 (amino or protobuf encoding)
    // 4. Broadcast via /cosmos/tx/v1beta1/txs
    //
    // Placeholder implementation:
    final uatomAmount = (amount * 1000000).toInt();
    throw UnimplementedError(
      'Cosmos transaction sending requires protobuf signing. '
      'Amount: $amount ATOM ($uatomAmount uatom) to $to. '
  ...
  String generateAddress(List<int> seed) {
    // BIP44 derivation path: m/44'/118'/0'/0/0
    // In a full implementation, this would:
    // 1. Derive the child key using BIP44 path with coin type 118
    // 2. Get the public key (secp256k1)
    // 3. SHA-256 then RIPEMD-160 of the public key
    // 4. Bech32 encode with "cosmos" prefix
    //
  ...
  bool validateAddress(String address) {
    // Cosmos Hub addresses: bech32 with "cosmos1" prefix, 44 chars total
    final regex = RegExp(r'^cosmos1[a-z0-9]{38}$');
    return regex.hasMatch(address);
  }

  @override
  Future<double> estimateFee() async {
    // Cosmos Hub typical fee: ~0.005 ATOM for a standard MsgSend
    return 0.005;
  }
