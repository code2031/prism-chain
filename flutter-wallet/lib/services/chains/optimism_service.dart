...
/// Popular tokens on Optimism with their contract addresses.
class OptimismToken {
  final String name;
  final String symbol;
  ...
  static const List<OptimismToken> popular = [
    OptimismToken(
      name: 'USD Coin',
      symbol: 'USDC',
...
/// Optimism (L2) chain service using Ethereum-compatible JSON-RPC.
///
/// Shares the same address format as Ethereum (BIP44 m/44'/60'/0'/0/0).
/// Uses Optimism RPC endpoints for balance and transaction queries.
class OptimismService extends ChainService {
  final http.Client _client;
  String _rpcEndpoint;
  int _requestId = 0;

  OptimismService({http.Client? client, String? rpcUrl})
      : _client = client ?? http.Client(),
        _rpcEndpoint = rpcUrl ?? AppConstants.optimismRpcUrl;

  @override
  String get chainName => 'Optimism';
  ...
  @override
  String get explorerUrl => 'https://optimistic.etherscan.io';
  ...
  Future<Map<String, dynamic>> _jsonRpc(String method, [List<dynamic>? params]) async {
    ...
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception('Optimism RPC HTTP ${response.statusCode}: ${response.body}');
      }
      ...
      if (json.containsKey('error')) {
        ...
        throw Exception('Optimism RPC Error: ${error['message']}');
      }

      return json;
    } catch (e) {
      if (e is Exception) rethrow;
      throw Exception('Optimism RPC connection failed: $e');
    }
  }
  ...
  Future<double> getBalance(String address) async {
    try {
      ...
      throw Exception('Failed to fetch Optimism balance: $e');
    }
  }

  /// Get the balance of a token on Optimism.
  ...
  Future<String> sendTransaction(String to, double amount) async {
    ...
    throw UnimplementedError(
      'Optimism transaction sending requires ECDSA signing. '
      'Amount: $amount ETH ($weiAmount Wei) to $to. '
      'Use a dedicated Ethereum/Optimism SDK for production transactions.',
    );
  }
      ...
      return [
        {
          ...
          'note': 'Transaction history requires Optimistic Etherscan API. Current block: $blockNum',
          'chain': 'optimism',
        },
      ];
    } catch (e) {
  ...
  Future<double> estimateFee() async {
    try {
      ...
      return 0.00005; // Optimism L2 fees are very low
    }
  }
